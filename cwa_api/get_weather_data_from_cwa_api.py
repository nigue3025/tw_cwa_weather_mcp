...
source_url="https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-D0047-093?Authorization="+api_key+"&format=JSON&"
...
try:
    ...
    for location in locations:
        ...
        Geo_code = location["Geocode"]
        Location_Name = location["LocationName"]
        print(Location_Name)  
        location_df=pd.DataFrame()
        for element in location["WeatherElement"]:
            element_name = element["ElementName"]
            if(element_name in key_mapping):
                rslts=function_mapping[key_mapping[element_name]](element)
                for rslt in rslts:
                    rslt["locationsName"]=Locations_Name
                    rslt['Distcode']=Geo_code
                    rslt['locationName']=Location_Name
                
                tempdf=pd.DataFrame(rslts)
                location_df=pd.concat([location_df,tempdf],ignore_index=True)
            
                total_rslts.extend(rslts)
                
                #rslt=convert_dct_to_dct_lst(rslt)
                #test=list(rslt.items())
                
                print(len(rslts))
            location_df=location_df.groupby(["date_time"], as_index=False).first()
        #df=df.groupby(["date_time" ,"locationsName","locationName", "Distcode"], as_index=False).first()
        df=pd.concat([df,location_df],ignore_index=True)
        print('total:'+str(len(total_rslts)))
